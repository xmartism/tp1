...
Použitie (z aktualneho priecinku):
    python3 plot_results.py <nazov_priecinku>/aggregated_results.csv

Argumenty:
    aggregated_results.csv  výstup zo skriptu aggregate_results.py

Výstup:
    <nazov_priecinku>/results_charts.html
"""
import pandas as pd
import json
import sys
...
if len(sys.argv) < 2:
    print("Použitie: python3 plot_results.py <nazov_priecinku>/aggregated_results.csv")
    sys.exit(1)
